  ...
  const renderMenuItems = () => {
    return navigation.map((item, index) => (
      <Link
        key={`${item.href}-${index}`}
        href={item.href}
        className='mx-2 cursor-pointer animation-hover inline-block relative text-black-500 hover:text-orange-500'
      >
        {item.name}
      </Link>
    ))
            ...
            <div className='flex flex-1 items-center sm:justify-between lg:gap-3'>
              ...
              <div className='gap-6 hidden lg:flex'>
                <div className='flex items-center gap-2'>
                  ...
                  />
                  <a
                    className='animation-hover relative text-lg font-medium text-black capitalize tracking-wide hover:text-orange-500 transition-all'
                    href='tel:0931888357'
                  >
                    {siteMetadata.phone}
                  </a>
